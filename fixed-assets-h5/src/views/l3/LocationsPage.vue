  ...
  <AppShell title="存放地点">
    ...
    <van-popup v-model:show="showAdd" position="bottom" round>
      <div class="popup-form">
        <div class="popup-title">{{ addForm.parentId ? '新增子地点' : '新增地点' }}</div>
        <van-field v-model="addForm.name" label="地点名称" placeholder="请输入" />
        <van-field v-model="addForm.code" label="地点编码" :placeholder="autoCodeHint" />
        <van-field 
    ...
    <van-popup v-model:show="showEdit" position="bottom" round>
      <div class="popup-form">
        <div class="popup-title">编辑地点</div>
        <van-field v-model="editForm.name" label="地点名称" />
        <van-field v-model="editForm.code" label="地点编码" />
        <div class="popup-actions">
...
import { getLocations, createLocation, updateLocation, deleteLocation } from "@/api/config"
...
async function onAddConfirm() {
  if (!addForm.value.name) return showToast("请输入地点名称")
  loadingStore.startLoading("创建中...")
  try {
    await createLocation(addForm.value)
    showToast("创建成功")
    showAdd.value = false
    loadData()
  } catch {
...
async function onEditConfirm() {
  ...
  try {
    await updateLocation(selectedNode.value.id, editForm.value)
    showToast("更新成功")
...
async function onDelete() {
  ...
  try {
    await deleteLocation(selectedNode.value.id)
    showToast("删除成功")
...
<style scoped>
.locations-page { min-height: 100vh; padding-bottom: 80px; }
